...
1. 拉取源代码
2. 安装依赖（`npm install` / `pnpm install`）。
3. 构建 popup（`npm run build:popup` / `pnpm build:popup`）。
4. 打开 Chrome / Edge 扩展管理页。
5. 开启“开发者模式”。
6. 选择“加载已解压的扩展程序”，目录选本项目文件夹。
7. 打开插件弹出窗口设置注入选项，启用注入和配置可选项大多数时候可以即时生效，如果遇到没有生效，或是关闭注入开关后请刷新页面。
