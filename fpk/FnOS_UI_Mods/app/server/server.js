...
function buildLaunchpadBridgeJs(presetConfig, templates) {
  ...
  return [
    ...
    '    try {',
    "      return new URL(rawKey.trim(), window.location.origin).pathname.toLowerCase();",
    '    } catch (_) {',
    ...
    '      const imageEl = getIconImageEl(cardEl);',
    "      const iconSrc = imageEl ? (imageEl.getAttribute(ORIG_DATA_SRC_ATTR) || imageEl.getAttribute(ORIG_SRC_ATTR) || imageEl.getAttribute('src') || imageEl.getAttribute('data-src') || '') : '';",
    '      map.set(key, {',
    '        key,',
